import { cn } from '@/lib/utils';
import { SourceCard } from './SourceCard'
import {WaldiumLogo} from './WaldiumLogo'

export default function WaldiumHero({
  className
}: {
  className?: string;
}) {

  const sources = [
    { id: 'github', label: 'Github Codebase', icon: 'github', color: '#1f2937' },
    { id: 'knowledge', label: 'Knowledge Base', icon: 'database', color: '#ea580c' },
    { id: 'sales', label: 'Sales Calls', icon: 'phone', color: '#059669' },
    { id: 'slack', label: 'Slack Huddles', icon: 'hash', color: '#7c3aed' },
    { id: 'web', label: 'Web Research', icon: 'globe', color: '#2563eb' },
  ];

  return (
    <div className={cn("relative min-w-[800px] w-full h-[900px] bg-white p-12 font-sans flex items-center flex-col mr-[160px]", className)}>
      {/* Background effects */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] bg-emerald-500/5 rounded-full blur-[120px] pointer-events-none" />

      {/* Main container - flexbox layout */}
      <div className="relative flex gap-0 xl:gap-8 h-full">

        {/* Left: Sources - Fixed width */}
        <div className="w-[250px] flex flex-col shrink-0 pt-40 gap-8">
          <div className="flex flex-col gap-5">
            {sources.map((source, index) => (
              <SourceCard
                key={source.id}
                icon={source.icon}
                label={source.label}
                color={source.color}
                index={index}
              />
            ))}
          </div>
        </div>

        {/* Center: Editor - Takes remaining space */}
        <div className="flex-1 flex flex-col relative max-w-[800px]" style={{ zIndex: 2 }}>

          {/* Notification Card */}
          <div className="w-full bg-white rounded-xl border border-neutral-200 mb-4 shadow-xs overflow-hidden">
            <div className="p-4 flex items-start gap-4">
              <div className="w-10 h-10 rounded-lg bg-emerald-50 flex items-center justify-center shrink-0 border border-emerald-100">
                <svg className="w-5 h-5 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-bold text-neutral-900 text-sm">Waldium</span>
                  <span className="text-xs text-neutral-400">Just now</span>
                </div>
                <h4 className="text-sm font-semibold text-neutral-800 mb-0.5">New Blog Post Drafted: Revolutionizing Product</h4>
                <p className="text-xs text-neutral-500">Based on your latest updates, I drafted &quot;Revolutionizing Product Marketing with Ambient Capture and AI&quot;</p>
              </div>
            </div>
            <div className="bg-neutral-50 px-4 py-2 border-t border-neutral-100 flex justify-end">
              <button className="text-xs font-semibold text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
                View Draft →
              </button>
            </div>
          </div>

          {/* Editor Window */}
          <div className="flex-1 bg-white border border-neutral-200 rounded-xl shadow-lg overflow-hidden flex flex-col ring-1 ring-neutral-100">

            {/* Editor Header */}
            <div className="h-12 border-b border-neutral-100 flex items-center px-4 justify-between bg-white shrink-0">
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" />
                </svg>
                <span className="text-sm font-medium text-neutral-600">Post Editor</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 rounded-md bg-emerald-50 text-emerald-600 text-[10px] font-bold uppercase tracking-wide border border-emerald-100">AUTO-SAVING</span>
                <button className="w-8 h-8 rounded-full hover:bg-neutral-100 flex items-center justify-center text-neutral-400">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
                  </svg>
                </button>
              </div>
            </div>

            {/* Toolbar */}
            <div className="h-11 border-b border-neutral-100 flex items-center px-4 gap-1 bg-neutral-50/30 shrink-0">
              <button className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600 text-sm font-semibold">B</button>
              <button className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600 text-sm italic">I</button>
              <button className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600 text-sm underline">U</button>
              <div className="w-[1px] h-5 bg-neutral-200 mx-2" />
              <button className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                </svg>
              </button>
              <button className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
              </button>
              <div className="w-[1px] h-5 bg-neutral-200 mx-2" />
              <button className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" />
                </svg>
              </button>
            </div>

            {/* Editor Content */}
            <div className="flex-1 p-10 bg-white overflow-auto relative">

              <h1 className="text-2xl font-semibold text-neutral-900 mb-6 tracking-tight">
                Revolutionizing Product Marketing with Ambient Capture and AI
              </h1>

              <div className="text-lg leading-relaxed text-neutral-800">
                <p className="mb-4">
                  This post explores how ambient capture transforms product marketing by enabling real-time, AI-friendly content updates that keep products visible and relevant.
                </p>
                <div className="inline">
                  <span>AI assistants like ChatGPT and Claude have become the main gateways through which developers and consumers discover software products. These smart helpers are shaping the way we find and evaluate new tools, but there&apos;s a catch: traditional marketing content can&apos;t keep up with the rapid pace of product updates and new features.</span>
                  <span className="inline-block w-[2px] h-6 bg-emerald-500 ml-1 animate-pulse"></span>
                  {/* AI Badge */}
                  <span className="inline-block w-[2px] relative h-6 overflow-visible">
                    <div className="absolute w-fit whitespace-nowrap bg-emerald-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-lg flex items-center gap-1.5"
                      style={{
                        top: 0,
                        left: 8
                      }}>
                      <WaldiumLogo width={12} height={12} />
                      Waldium
                    </div>
                  </span>
                </div>
              </div>

              <div className="absolute top-0 left-0 w-full h-full flex flex-col p-4" style={{
                background: "linear-gradient(\n  to bottom right,\n  rgba(255, 255, 255, 0) 0%,\n  rgba(255, 255, 255, 0) 40%,\n  #ffffff 80%,\n  #ffffff 100%\n)"
              }} />
            </div>

            {/* Editor Footer */}
            <div className="h-9 border-t border-neutral-100 bg-neutral-50 flex items-center px-4 gap-4 text-[10px] text-neutral-400 font-medium shrink-0">
              <span>Ln 12, Col 45</span>
              <span>Markdown</span>
              <span className="ml-auto flex items-center gap-1.5 text-emerald-600">
                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                Synced to Cloud
              </span>
            </div>
          </div>
        </div>

        {/* Right: Analytics - Positioned absolutely to overlap */}
        <div className="absolute right-0 translate-x-1/2 bottom-12 w-[320px]" style={{ zIndex: 3 }}>

          <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-2xl flex flex-col gap-0">
            <div className="flex items-center justify-between m-0 mb-6">
              <div>
                <h4 className="text-neutral-900 text-lg font-semibold">Brand Visibility</h4>
                <p className="text-xs text-neutral-500 font-normal mt-0.5">Percentage of AI answers that mentions Waldium</p>
              </div>
              <div className="w-8 min-w-8 h-8 bg-emerald-50 rounded-full flex items-center justify-center border border-emerald-100 shrink-0">
                <svg className="w-4 h-4 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                </svg>
              </div>
            </div>

            {/* Chart */}
            <div className="h-fit w-full relative mb-3">
              <svg className="w-full h-20" viewBox="0 0 280 140" preserveAspectRatio="none">
                <defs>
                  <linearGradient id="areaGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#10B981" stopOpacity={0.35}/>
                    <stop offset="100%" stopColor="#10B981" stopOpacity={0.02}/>
                  </linearGradient>
                </defs>
                {/* Grid lines - horizontal */}
                <line x1="0" y1="0" x2="280" y2="0" stroke="#e5e7eb" strokeWidth="1" opacity="0.5"/>
                <line x1="0" y1="35" x2="280" y2="35" stroke="#e5e7eb" strokeWidth="1" opacity="0.5"/>
                <line x1="0" y1="70" x2="280" y2="70" stroke="#e5e7eb" strokeWidth="1" opacity="0.5"/>
                <line x1="0" y1="105" x2="280" y2="105" stroke="#e5e7eb" strokeWidth="1" opacity="0.5"/>
                <line x1="0" y1="140" x2="280" y2="140" stroke="#e5e7eb" strokeWidth="1" opacity="0.5"/>

                {/* Area fill */}
                <path
                  d="M 0 100 Q 25 95 50 98 T 100 92 T 150 88 T 200 82 T 250 78 L 280 75 L 280 140 L 0 140 Z"
                  fill="url(#areaGradient)"
                />
                {/* Main line */}
                <path
                  d="M 0 100 Q 25 95 50 98 T 100 92 T 150 88 T 200 82 T 250 78 L 280 75"
                  stroke="#10B981"
                  strokeWidth="2.5"
                  fill="none"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </div>

            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between m-0">
                <div className="flex flex-col">
                  <span className="text-[10px] text-neutral-400 font-semibold uppercase">Visibility Score</span>
                  <span className="text-2xl font-bold text-neutral-900">87.6%</span>
                </div>
                <div className="flex items-center gap-1.5 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100">
                  <svg className="w-3 h-3 text-emerald-600" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M5.293 9.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L11 7.414V15a1 1 0 11-2 0V7.414L6.707 9.707a1 1 0 01-1.414 0z" clipRule="evenodd" />
                  </svg>
                  <span className="text-xs text-emerald-700 font-bold">1.27%</span>
                </div>
              </div>

              {/* Tracking label */}
              <div className="flex items-center gap-2 text-xs text-neutral-500">
                <span className="font-medium">Tracking</span>
                <div className="flex items-center gap-1.5">
                  {/* Claude */}
                  <div className="w-5 h-5 bg-neutral-100 rounded-full flex items-center justify-center overflow-hidden">
                    <svg className="w-3 h-3" viewBox="0 0 1200 1200" fill="#d97757">
                      <path d="M 233.959793 800.214905 L 468.644287 668.536987 L 472.590637 657.100647 L 468.644287 650.738403 L 457.208069 650.738403 L 417.986633 648.322144 L 283.892639 644.69812 L 167.597321 639.865845 L 54.926208 633.825623 L 26.577238 627.785339 L 3.3e-05 592.751709 L 2.73832 575.27533 L 26.577238 559.248352 L 60.724873 562.228149 L 136.187973 567.382629 L 249.422867 575.194763 L 331.570496 580.026978 L 453.261841 592.671082 L 472.590637 592.671082 L 475.328857 584.859009 L 468.724915 580.026978 L 463.570557 575.194763 L 346.389313 495.785217 L 219.543671 411.865906 L 153.100723 363.543762 L 117.181267 339.060425 L 99.060455 316.107361 L 91.248367 266.01355 L 123.865784 230.093994 L 167.677887 233.073853 L 178.872513 236.053772 L 223.248367 270.201477 L 318.040283 343.570496 L 441.825592 434.738342 L 459.946411 449.798706 L 467.194672 444.64447 L 468.080597 441.020203 L 459.946411 427.409485 L 392.617493 305.718323 L 320.778564 181.932983 L 288.80542 130.630859 L 280.348999 99.865845 C 277.369171 87.221436 275.194641 76.590698 275.194641 63.624268 L 312.322174 13.20813 L 332.8591 6.604126 L 382.389313 13.20813 L 403.248352 31.328979 L 434.013519 101.71814 L 483.865753 212.537048 L 561.181274 363.221497 L 583.812134 407.919434 L 595.892639 449.315491 L 600.40271 461.959839 L 608.214783 461.959839 L 608.214783 454.711609 L 614.577271 369.825623 L 626.335632 265.61084 L 637.771851 131.516846 L 641.718201 93.745117 L 660.402832 48.483276 L 697.530334 24.000122 L 726.52356 37.852417 L 750.362549 72 L 747.060486 94.067139 L 732.886047 186.201416 L 705.100708 330.52356 L 686.979919 427.167847 L 697.530334 427.167847 L 709.61084 415.087341 L 758.496704 350.174561 L 840.644348 247.490051 L 876.885925 206.738342 L 919.167847 161.71814 L 946.308838 140.29541 L 997.61084 140.29541 L 1035.38269 196.429626 L 1018.469849 254.416199 L 965.637634 321.422852 L 921.825562 378.201538 L 859.006714 462.765259 L 819.785278 530.41626 L 823.409424 535.812073 L 832.75177 534.92627 L 974.657776 504.724915 L 1051.328979 490.872559 L 1142.818848 475.167786 L 1184.214844 494.496582 L 1188.724854 514.147644 L 1172.456421 554.335693 L 1074.604126 578.496765 L 959.838989 601.449829 L 788.939636 641.879272 L 786.845764 643.409485 L 789.261841 646.389343 L 866.255127 653.637634 L 899.194702 655.409424 L 979.812134 655.409424 L 1129.932861 666.604187 L 1169.154419 692.537109 L 1192.671265 724.268677 L 1188.724854 748.429688 L 1128.322144 779.194641 L 1046.818848 759.865845 L 856.590759 714.604126 L 791.355774 698.335754 L 782.335693 698.335754 L 782.335693 703.731567 L 836.69812 756.885986 L 936.322205 846.845581 L 1061.073975 962.81897 L 1067.436279 991.490112 L 1051.409424 1014.120911 L 1034.496704 1011.704712 L 924.885986 929.234924 L 882.604126 892.107544 L 786.845764 811.48999 L 780.483276 811.48999 L 780.483276 819.946289 L 802.550415 852.241699 L 919.087341 1027.409424 L 925.127625 1081.127686 L 916.671204 1098.604126 L 886.469849 1109.154419 L 853.288696 1103.114136 L 785.073914 1007.355835 L 714.684631 899.516785 L 657.906067 802.872498 L 650.979858 806.81897 L 617.476624 1167.704834 L 601.771851 1186.147705 L 565.530212 1200 L 535.328857 1177.046997 L 519.302124 1139.919556 L 535.328857 1066.550537 L 554.657776 970.792053 L 570.362488 894.68457 L 584.536926 800.134277 L 592.993347 768.724976 L 592.429626 766.630859 L 585.503479 767.516968 L 514.22821 865.369263 L 405.825531 1011.865906 L 320.053711 1103.677979 L 299.516815 1111.812256 L 263.919525 1093.369263 L 267.221497 1060.429688 L 287.114136 1031.114136 L 405.825531 880.107361 L 477.422913 786.52356 L 523.651062 732.483276 L 523.328918 724.671265 L 520.590698 724.671265 L 205.288605 929.395935 L 149.154434 936.644409 L 124.993355 914.01355 L 127.973183 876.885986 L 139.409409 864.80542 L 234.201385 799.570435 L 233.879227 799.8927 Z"/>
                    </svg>
                  </div>

                  {/* ChatGPT */}
                  <div className="w-5 h-5 bg-neutral-100 rounded-full flex items-center justify-center overflow-hidden">
                    <svg className="w-3 h-3" viewBox="0 0 320 320" fill="#10a37f">
                      <path d="m297.06 130.97c7.26-21.79 4.76-45.66-6.85-65.48-17.46-30.4-52.56-46.04-86.84-38.68-15.25-17.18-37.16-26.95-60.13-26.81-35.04-.08-66.13 22.48-76.91 55.82-22.51 4.61-41.94 18.7-53.31 38.67-17.59 30.32-13.58 68.54 9.92 94.54-7.26 21.79-4.76 45.66 6.85 65.48 17.46 30.4 52.56 46.04 86.84 38.68 15.24 17.18 37.16 26.95 60.13 26.8 35.06.09 66.16-22.49 76.94-55.86 22.51-4.61 41.94-18.7 53.31-38.67 17.57-30.32 13.55-68.51-9.94-94.51zm-120.28 168.11c-14.03.02-27.62-4.89-38.39-13.88.49-.26 1.34-.73 1.89-1.07l63.72-36.8c3.26-1.85 5.26-5.32 5.24-9.07v-89.83l26.93 15.55c.29.14.48.42.52.74v74.39c-.04 33.08-26.83 59.9-59.91 59.97zm-128.84-55.03c-7.03-12.14-9.56-26.37-7.15-40.18.47.28 1.3.79 1.89 1.13l63.72 36.8c3.23 1.89 7.23 1.89 10.47 0l77.79-44.92v31.1c.02.32-.13.63-.38.83l-64.41 37.19c-28.69 16.52-65.33 6.7-81.92-21.95zm-16.77-139.09c7-12.16 18.05-21.46 31.21-26.29 0 .55-.03 1.52-.03 2.2v73.61c-.02 3.74 1.98 7.21 5.23 9.06l77.79 44.91-26.93 15.55c-.27.18-.61.21-.91.08l-64.42-37.22c-28.63-16.58-38.45-53.21-21.95-81.89zm221.26 51.49-77.79-44.92 26.93-15.54c.27-.18.61-.21.91-.08l64.42 37.19c28.68 16.57 38.51 53.26 21.94 81.94-7.01 12.14-18.05 21.44-31.2 26.28v-75.81c.03-3.74-1.96-7.2-5.2-9.06zm26.8-40.34c-.47-.29-1.3-.79-1.89-1.13l-63.72-36.8c-3.23-1.89-7.23-1.89-10.47 0l-77.79 44.92v-31.1c-.02-.32.13-.63.38-.83l64.41-37.16c28.69-16.55 65.37-6.7 81.91 22 6.99 12.12 9.52 26.31 7.15 40.1zm-168.51 55.43-26.94-15.55c-.29-.14-.48-.42-.52-.74v-74.39c.02-33.12 26.89-59.96 60.01-59.94 14.01 0 27.57 4.92 38.34 13.88-.49.26-1.33.73-1.89 1.07l-63.72 36.8c-3.26 1.85-5.26 5.31-5.24 9.06l-.04 89.79zm14.63-31.54 34.65-20.01 34.65 20v40.01l-34.65 20-34.65-20z"/>
                    </svg>
                  </div>

                  {/* Gemini */}
                  <div className="w-5 h-5 bg-neutral-100 rounded-full flex items-center justify-center">
                    <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
                      <defs>
                        <linearGradient id="geminiGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                          <stop offset="0%" style={{ stopColor: '#4285F4' }} />
                          <stop offset="50%" style={{ stopColor: '#9B72CB' }} />
                          <stop offset="100%" style={{ stopColor: '#D96570' }} />
                        </linearGradient>
                      </defs>
                      <path fill="url(#geminiGradient)" fillRule="evenodd" d="M20.616 10.835a14.147 14.147 0 01-4.45-3.001 14.111 14.111 0 01-3.678-6.452.503.503 0 00-.975 0 14.134 14.134 0 01-3.679 6.452 14.155 14.155 0 01-4.45 3.001c-.65.28-1.318.505-2.002.678a.502.502 0 000 .975c.684.172 1.35.397 2.002.677a14.147 14.147 0 014.45 3.001 14.112 14.112 0 013.679 6.453.502.502 0 00.975 0c.172-.685.397-1.351.677-2.003a14.145 14.145 0 013.001-4.45 14.113 14.113 0 016.453-3.678.503.503 0 000-.975 13.245 13.245 0 01-2.003-.678z"/>
                    </svg>
                  </div>

                  {/* Grok */}
                  <div className="w-5 h-5 bg-neutral-100 rounded-full flex items-center justify-center overflow-hidden">
                    <svg className="w-3 h-3" viewBox="0 0 48 48" fill="#000000">
                      <path d="M18.542 30.532l15.956-11.776c.783-.576 1.902-.354 2.274.545 1.962 4.728 1.084 10.411-2.819 14.315-3.903 3.901-9.333 4.756-14.299 2.808l-5.423 2.511c7.778 5.315 17.224 4 23.125-1.903 4.682-4.679 6.131-11.058 4.775-16.812l.011.011c-1.966-8.452.482-11.829 5.501-18.735C47.759 1.332 47.88 1.166 48 1l-6.602 6.599V7.577l-22.86 22.958M15.248 33.392c-5.582-5.329-4.619-13.579.142-18.339 3.521-3.522 9.294-4.958 14.331-2.847l5.412-2.497c-.974-.704-2.224-1.46-3.659-1.994-6.478-2.666-14.238-1.34-19.505 3.922C6.904 16.701 5.31 24.488 8.045 31.133c2.044 4.965-1.307 8.48-4.682 12.023C2.164 44.411.967 45.67 0 47l15.241-13.608"/>
                    </svg>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>

    </div>
  );
}
